...
export async function loader_services(param: {
    ...
    await new Promise((resolve, reject) => {
      fs.createReadStream(filePath)
        ...
        .on('data', async (data: CsvData) => {
          try {
            await prismaService.services.create({
              data: {
                row_id: data.row_id,
                curr_service: data.curr_service,
                hadm_id: data.hadm_id,
                prev_service: data.prev_service,
